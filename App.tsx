  ...
  onLoadClick: () => void;
    ...
    <div className="text-center bg-[#CED2D9]/50 dark:bg-[#464D56]/50 rounded-xl border-2 border-dashed border-[#464D56] dark:border-[#CED2D9] p-8 md:p-12">
        <div className="flex flex-col items-center">
            ...
            <p className="text-[#464D56] dark:text-[#CED2D9] mb-4">Load a saved moodboard `.json` file to begin.</p>
            <button
              onClick={onLoadClick}
              className="px-6 py-2 bg-[#44A0D1] dark:bg-[#54C1FB] text-white font-semibold rounded-lg shadow-md hover:bg-[#44A0D1]/90 dark:hover:bg-[#54C1FB]/90 focus:outline-none focus:ring-2 focus:ring-[#44A0D1] dark:focus:ring-[#54C1FB] focus:ring-opacity-75 transition-transform transform hover:scale-105 flex items-center gap-2"
            >
  ...
  const handleLoadCanvas = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (loadedData.version === 9 || loadedData.version === 8 || loadedData.version === 7) {
            const state = loadedData as SaveState;
            ...
            setItemConnections(state.itemConnections || {});
            setOutputCards(state.outputCards || []);
        } else {
            throw new Error('Invalid or unsupported save file format.');
        }
      } catch (error) {
      ...
      <main className="flex-grow relative">
        {hasContent ? (
          ...
          <div className="w-full h-full flex items-center justify-center p-4">
              <LoadMoodboardPrompt onLoadClick={() => loadInputRef.current?.click()} />
